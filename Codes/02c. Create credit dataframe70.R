...
## Panel data for cycles

country='AR'
df0 = read.table(sprintf("../Data/Processed/GeneratedCycles_%s.csv",country), header=TRUE, sep=",")
varlist = colnames(df0)
#varlist = varlist[-c((length(varlist)-5):(length(varlist)-1))]
df0=df0[varlist]
...
write.table(df0, "../Data/input/credit_gaps_allcountry.csv", sep=',', row.names=FALSE)
