one_sided_hp_filter_kalman <- function(y,lambda,x_user,P_user,discard){
#####################################################
#
...
if (nargin < 4 || isempty(x_user), x <- [2*y(1,k)-y(2,k); 3*y(1,k)-2*y(2,k)]; else x <- x_user(:,k);}){#If the user didn't provide an intial value for state estimate, extrapolate back two periods from the observations
if (nargin < 4 || isempty(P_user), P <-  [1e5 0;0 1e5]; else P <- P_user{k}; }){#If the user didn't provide an intial value for the MSE, set a rather high one
  ...
  for (j in 1:T){#Get the estimates for each period
       ...
       ytr}(j,k) <- x(2)#The second element of the state is the estimate of the trend
  }
}

if (nargout==2){#Should the user have requested a second output
    ycycle <- y-ytr}#The second output will be the deviations from the HP trend
}

if (nargin==5,){#If the user provided a discard parameter
    ytr} <- ytr}(discard+1:},:)#Remove the first "discard" periods from the trend series
    if (nargout==2){#Should the user have requested a second output
        ycycle <- ycycle(discard+1:},:)#The second output will be the deviations from the HP trend, likewise with  first "discard" periods removed
    }
}
}

kalman_update <- function(F,H,Q,R,obs,x,P){
   ####
   #
