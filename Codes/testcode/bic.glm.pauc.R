                          ...
                          ...) 
{
  ...
  bic1 <- as.numeric((1 - pauc) * 10 - df * log(nobs) - 2 * log(prior))
  ...
  probne0 <- round(100 * t(which) %*% as.matrix(postprob1), 
                   1)
  nmod <- length(bic1)
  ...
  SDbi <- sqrt(postprob1 %*% (sebiMk^2) + postprob1 %*% ((EbiMk - 
                                                          Ebimat)^2))
