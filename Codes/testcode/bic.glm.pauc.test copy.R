...
bic.glm.pauc <- function (x, y, glm.family, wt = rep(1, nrow(x)), strict = FALSE, 
                          ...
                          ...) 
{
  ...
  dropcols <- function(x, y, glm.family, wt, maxCols = 30) {
    ...
    while (length(glm.out$coefficients) > maxCol) {
      ...
      lrtname <- if (!is.null(dropglm$LRT)) 
        "LRT"
      else "scaled dev."
    ...
    xx <- data.frame(matrix(rep(NA, times = new.nvar * nrow(x.df)), 
                            ncol = new.nvar))
    colnames(xx) <- sapply(colnames(x.df), function(s) substring(s, 
                                                                 3, nchar(s)))
  ...
  pauc <- rep(0, nmod)
  bic1 <- rep(0,nmod)
  postprob1 <- rep(0,nmod)
  xdf <- x.df
  colnames(xdf) <- sapply(colnames(xdf), function(s) substring(s, 
                                                               3, nchar(s)))
  for (k in 1:nmod) {
    if (sum(which[k, ]) == 0) {
      ...
      if (ncol(x.df) != 1) {
        colnames(x.df) <- sapply(colnames(x.df), function(s) substring(s, 
                                                                       3, nchar(s)))
      }
    ...
    test_roc = pROC::roc(y ~ test_prob, plot = FALSE, print.auc = FALSE, levels = c(1,0)
                         , direction = ">")
  ...
  #bic1 <- as.numeric(2*(1-pauc)*10 - df * log(nobs) - 2 * log(prior))
  bic1 <- as.numeric((1 - pauc) - (df * log(nobs)) - (2 * log(prior)))
  
  if (occam.window) 
    occam <- bic1 - min(bic1) < 2 * log(OR)
  else occam = rep(TRUE, length(bic1))
  dev <- dev[occam]
  df <- df[occam]
  size <- size[occam]
  label <- label[occam]
  ...
  pauc <- pauc[occam]
  postprob1 <- exp(-0.5 * (bic1 - min(bic1)))/sum(exp(-0.5 * (bic1 - min(bic1))))
  order.bic <- order(pauc)
  #order.bic <- order(bic1, size, label)
  dev <- dev[order.bic]
  df <- df[order.bic]
  size <- size[order.bic]
  ...
  if (strict & (nmod != 1)) {
    ...
    bic1 <- bic1[occam]
    prior <- prior[occam]
    postprob1 <- postprob1[occam]
    postprob1 <- postprob1/sum(postprob1)
    model.fits <- model.fits[occam]
  }
  bic1 <- bic1 + 2 * log(prior)
  probne0 <- round(100 * t(which) %*% as.matrix(postprob1),1)
  nmod <- length(bic1)
  ...
  SDbi <- sqrt(postprob1 %*% (sebiMk^2) + postprob1 %*% ((EbiMk - Ebimat)^2))
  CEbi <- CSDbi <- rep(0, nvar)
  for (i in (1:ncol(x))) {
    sel <- which[, i]
    if (sum(sel) > 0) {
      cpp <- rbind(postprob1[sel]/sum(postprob1[sel]))
      CEbi[glm.assign[[i + 1]]] <- as.numeric(cpp %*% EbiMk[sel, glm.assign[[i + 1]]])
      CSDbi[glm.assign[[i + 1]]] <- sqrt(cpp %*% (sebiMk[sel, glm.assign[[i + 1]]]^2) + cpp %*% ((EbiMk[sel, 
  ...
  if (pad != "") 
    varNames <- sapply(varNames, function(z) substring(z, 
                                                       1, nchar(z) - 2))
  names(varNames) <- NULL
  ...
  result <- list(postprob1 = postprob1, label = label, deviance = dev, 
                 size = size, bic1 = bic1, prior.param = prior.param, prior.model.weights = prior/prior.weight.denom, 
