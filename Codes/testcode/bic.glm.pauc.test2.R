...
"bic.glm.pauc" <-
            ...
            ...) 
  {
    ...
    if (ncol(x) > 2) {
      ...
      lrt <- bIb - (a$r2 * bIb)
      bic <- lrt + 2 * (a$size) * log(nobs) - 2 * log(prior)
    ...
    bic <- (-1) * ((pauc - 0.5) * 500 * 2) + 2 * rank0 * log(nobs) - 2 * log(prior)
    if (occam.window) 
